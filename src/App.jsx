...
const sb = {
  ...
  async signUp(email, password, nombre, nombreEmpresa) {
    const r = await fetch(this.authUrl("signup"), {
      method:"POST", headers:{"Content-Type":"application/json","apikey":SUPABASE_KEY},
      body: JSON.stringify({ email, password, data:{ nombre, nombreEmpresa } })
    });
    const data = await r.json();
    if (!r.ok) throw new Error(data.error_description || data.msg || "Error al registrarse");
    return data;
  },
  async signIn(email, password) {
    ...
    const data = await r.json();
    if (!r.ok) throw new Error(data.error_description || data.msg || "Email o contraseña incorrectos");
    return data;
  },
  async signOut(token) {
  ...
  withToken(token) {
    return { ...this, headers: { ...this.headers, "Authorization": `Bearer ${token}` } };
  }
};
...
// ─── STORAGE LOCAL (fallback si no hay sesión) ────────────────────────────────
const LS = {
...
export default function App() {
  ...
  // Init: if session stored, load empresa
  useEffect(() => {
    if (session) {
  ...
  const loadEmpresa = async (sess) => {
    try {
      const api = sb.withToken(sess.access_token);
      const perfiles = await api.query("perfiles", `?id=eq.${sess.user.id}&select=*,empresas(*)`);
  ...
  const handleRegister = async (email, password, nombre, nombreEmpresa) => {
    setLoading(true);
    try {
      const data = await sb.signUp(email, password, nombre, nombreEmpresa);
      if (data.user) {
        // Create empresa and perfil via RPC workaround using service
        const api = sb.withToken(data.access_token || SUPABASE_KEY);
        // Insert empresa
...
function AuthScreen({ view, setView, onLogin, onRegister, loading }) {
  ...
  const inp = { width:"100%", border:"1px solid #ddd", borderRadius:10, padding:"11px 14px", fontSize:14, outline:"none", boxSizing:"border-box", marginBottom:10, fontFamily:"inherit" };

  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0F2444 0%,#1B3E6E 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{width:"100%",maxWidth:400}}>
        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{fontSize:48,marginBottom:8}}>🏗️</div>
          <div style={{color:"#fff",fontWeight:800,fontSize:28,letterSpacing:-0.5}}>{APP_NAME}</div>
        ...
        <div style={{background:"#fff",borderRadius:20,padding:28,boxShadow:"0 20px 60px rgba(0,0,0,.3)"}}>
          <div style={{display:"flex",marginBottom:24,background:"#F1F5F9",borderRadius:10,padding:3}}>
            {[["login","Iniciar sesión"],["register","Crear cuenta"]].map(([v,l])=>(
              <button key={v} onClick={()=>{setView(v);setError("");}} style={{flex:1,border:"none",borderRadius:8,padding:"9px",cursor:"pointer",fontWeight:600,fontSize:13,background:view===v?"#fff":"transparent",color:view===v?"#0F2444":"#64748b",boxShadow:view===v?"0 1px 4px rgba(0,0,0,.1)":"none",transition:"all .15s"}}>
                {l}
              </button>
            ))}
          </div>
          ...
          <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Contraseña" style={{...inp,marginBottom:0}}
            onKeyDown={e=>e.key==="Enter"&&handleSubmit()}/>

          {error && <div style={{color:"#ef4444",fontSize:12,marginTop:8,padding:"8px 10px",background:"#fff5f5",borderRadius:7}}>{error}</div>}

          <button onClick={handleSubmit} disabled={loading} style={{width:"100%",marginTop:16,background:loading?"#94a3b8":"#0F2444",color:"#fff",border:"none",borderRadius:10,padding:"13px",fontWeight:700,fontSize:15,cursor:loading?"not-allowed":"pointer",transition:"background .15s"}}>
...
//  MAIN APP (logged in)
// ═══════════════════════════════════════════════════════════════════════════════
function MainApp({ session, empresa, onLogout }) {
  const api = sb.withToken(session.access_token);
  ...
  const [fModal,  setFModal]  = useState(null); // factura form
  const [fView,   setFView]   = useState(null); // ver factura
  ...
  const loadAll = async () => {
    ...
    try {
      const [obras, puestos, trabajadores, peonadas, anticipos, clientes, facturas] = await Promise.all([
        api.query("obras",         `?empresa_id=eq.${eId}&order=created_at`),
        api.query("puestos",       `?empresa_id=eq.${eId}&order=created_at`),
        api.query("trabajadores",  `?empresa_id=eq.${eId}&activo=eq.true&order=created_at`),
        api.query("peonadas",      `?empresa_id=eq.${eId}`),
        api.query("anticipos",     `?empresa_id=eq.${eId}`),
        api.query("clientes",      `?empresa_id=eq.${eId}&order=nombre`),
        api.query("facturas",      `?empresa_id=eq.${eId}&order=created_at.desc`),
      ]);
  ...
  const toggleDay = async (wId, d) => {
    ...
    try {
      if (existing) {
        await api.delete("peonadas", existing.id);
        setData(prev => ({...prev, peonadas: prev.peonadas.filter(p=>p.id!==existing.id)}));
      } else {
        const nueva = await api.insert("peonadas", { empresa_id:eId, trabajador_id:wId, fecha });
  ...
  const calcWorker = (w) => {
    ...
    const antList = data.anticipos.filter(a => {
      ...
      return a.trabajador_id===w.id && fecha.getMonth()===selMonth && fecha.getFullYear()===selYear;
    });
  ...
  const savePuesto = async (form, editId) => {
    try {
      ...
      if (editId) {
        const upd = await api.update("puestos", editId, payload);
        setData(prev => ({...prev, puestos:prev.puestos.map(p=>p.id===editId?upd:p)}));
        showToast("Puesto actualizado ✓");
      } else {
        const nuevo = await api.insert("puestos", payload);
  ...
  const saveObra = async (form, editId) => {
    try {
      ...
      if (editId) {
        const upd = await api.update("obras", editId, payload);
        setData(prev => ({...prev, obras:prev.obras.map(o=>o.id===editId?upd:o)}));
        showToast("Obra actualizada ✓");
      } else {
        const nuevo = await api.insert("obras", payload);
  ...
  const addAnticipo = async (wId, form) => {
    try {
      const nuevo = await api.insert("anticipos", { empresa_id:eId, trabajador_id:wId, importe:parseFloat(form.importe), fecha:form.fecha, nota:form.nota||null });
  ...
  const saveFactura = async (form, editId) => {
    try {
      ...
      if (editId) {
        const upd = await api.update("facturas", editId, payload);
        setData(prev => ({...prev, facturas:prev.facturas.map(f=>f.id===editId?upd:f)}));
        showToast("Factura actualizada ✓");
      } else {
        const nueva = await api.insert("facturas", payload);
  ...
  const saveCliente = async (form, editId) => {
    try {
      const payload = { empresa_id:eId, ...form };
      if (editId) {
        const upd = await api.update("clientes", editId, payload);
        setData(prev => ({...prev, clientes:prev.clientes.map(c=>c.id===editId?upd:c)}));
        showToast("Cliente actualizado ✓");
      } else {
        const nuevo = await api.insert("clientes", payload);
  ...
  const changeEstadoFactura = async (id, estado) => {
    try {
      const upd = await api.update("facturas", id, {estado});
  ...
  const showMonthBar = !["puestos","facturas","clientes"].includes(tab) || (tab==="facturas"&&!fView) || tab==="resumen";
    ...
    <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",minHeight:"100vh",background:"#F1F5F9",color:"#1e293b",overflowX:"hidden"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#0F2444,#1B3E6E)",height:56,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 16px",boxShadow:"0 2px 12px rgba(0,0,0,.3)"}}>
        ...
        <div style={{position:"relative"}}>
          ...
          {menuOpen && (
            <div style={{position:"absolute",right:0,top:40,background:"#fff",borderRadius:10,boxShadow:"0 8px 24px rgba(0,0,0,.2)",padding:8,minWidth:160,zIndex:99}}>
              <div style={{padding:"6px 10px",fontSize:12,color:"#94a3b8",borderBottom:"1px solid #f0f4f8",marginBottom:4}}>
                {empresa.email}
              </div>
      ...
      <div style={{display:"flex",background:"#fff",borderBottom:"1px solid #E2E8F0",overflowX:"auto"}}>
        {NAV.map(([v,ic,l])=>(
          <button key={v} onClick={()=>{setTab(v);setFView(null);setMenuOpen(false);}} style={{
            padding:"10px 11px",border:"none",background:"none",cursor:"pointer",
            fontWeight:tab===v?700:400,color:tab===v?"#0F2444":"#64748b",
            borderBottom:tab===v?"3px solid #0F2444":"3px solid transparent",
            fontSize:11,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4
          }}>{ic} {l}</button>
        ))}
      </div>
      ...
      <div style={{padding:12}}>
        ...
        {tab==="asistencia" && (
          <div>
            {data.trabajadores.length===0 && <EmptyMsg icon="👷" text="No hay trabajadores" sub="Ve a la pestaña Trabajadores para añadir" />}
        ...
        {tab==="anticipos" && (
          <div>
            ...
            {data.trabajadores.length===0 && <EmptyMsg icon="👷" text="No hay trabajadores" />}
            {data.trabajadores.map(w => {
              const calc  = calcWorker(w);
              const color = puestoColor(w.puesto_id);
              return (
        ...
        {tab==="facturas" && !fView && (
          <FacturasLista
            facturas={data.facturas} clientes={data.clientes}
            calcTotals={calcFacturaTotals}
            onNew={()=>setFModal({mode:"new"})}
            onView={f=>{setFView(f);}}
        ...
        {tab==="facturas" && fView && (
          <FacturaVer
            ...
            calcTotals={calcFacturaTotals}
            empresa={empresa}
            onBack={()=>setFView(null)}
            onEdit={f=>setFModal({mode:"edit",f})}
        ...
        {tab==="resumen" && (
          <ResumenTab
            totales={totales} trabajadores={data.trabajadores} obras={obraStats}
            facturas={data.facturas} calcFacturaTotals={calcFacturaTotals}
            puestoColor={puestoColor} badge={badge} card={card}
            selMonth={selMonth} selYear={selYear} calcWorker={calcWorker}
            puestos={data.puestos} obrasList={data.obras} fmt={fmt} MONTHS={MONTHS}
          />
        )}
      </div>
      ...
      {aModal && (
        <AnticiposModal
          worker={data.trabajadores.find(w=>w.id===aModal)}
          calc={calcWorker(data.trabajadores.find(w=>w.id===aModal)||{})}
          selMonth={selMonth} selYear={selYear}
          onClose={()=>setAModal(null)}
          onAdd={addAnticipo} onDelete={deleteAnticipo}
          inp={inp} MONTHS={MONTHS} fmt={fmt}
        />
      )}
      ...
      {fModal && (
        <FacturaForm
          modal={fModal} clientes={data.clientes}
          nextNum={nextFacturaNum(data.facturas)}
          empresa={empresa}
...
function CalGrid({ monthDays, wId, color, isDayWorked, toggleDay }) {
  ...
  monthDays.forEach(function(dayObj) {
    ...
    cells.push(
      <button key={d} onClick={()=>toggleDay(wId,d)} style={{
        border:active?"2px solid "+color:"1.5px solid #e2e8f0",
        borderRadius:7,padding:"5px 2px",cursor:"pointer",textAlign:"center",
        background:active?color:isWE?"#f8fafc":"#fff",
        color:active?"#fff":isWE?"#b0b8c8":"#334155",
        fontSize:12,fontWeight:active?700:400,lineHeight:1.3,
        boxShadow:active?"0 2px 5px "+color+"55":"none",
      }}>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:50}}>
      <div style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:24,width:"100%",maxWidth:480,boxShadow:"0 -4px 24px rgba(0,0,0,.2)",maxHeight:"82vh",overflowY:"auto",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
        ...
        <div style={{fontWeight:600,fontSize:13,color:"#475569",marginBottom:8}}>
          Anticipos de {MONTHS[selMonth]} · Total: {fmt(calc ? calc.antic : 0)}
        </div>
...
function FacturasLista({ facturas, calcTotals, onNew, onView }) {
  ...
  const filtered = facturas.filter(f => {
    ...
    const matchQ = !q || f.numero.toLowerCase().includes(q) || (snap.nombre||"").toLowerCase().includes(q);
    const matchF = filter==="todas" || f.estado===filter;
    ...
    <div>
      ...
      <div style={{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 1px 6px rgba(0,0,0,.08)"}}>
        ...
        {filtered.map((f,i)=>{
          ...
          const snap=f.cliente_snapshot||{};
          return (
            <div key={f.id} onClick={()=>onView(f)} style={{padding:"13px 16px",borderBottom:i<filtered.length-1?"1px solid #F1F5F9":"none",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:10,transition:"background .1s"}}
  ...
  return (
    <div>
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap"}}>
        ...
        <button onClick={handlePrint} style={{background:"#059669",color:"#fff",border:"none",borderRadius:9,padding:"8px 14px",cursor:"pointer",fontWeight:700,fontSize:13}}>🖨️ Imprimir / PDF</button>
        <select value={factura.estado} onChange={e=>onEstado(factura.id,e.target.value)}
          style={{border:`2px solid ${est?.dot}`,borderRadius:9,padding:"8px 12px",fontSize:13,background:est?.bg,color:est?.col,fontWeight:700,cursor:"pointer"}}>
      ...
      <div id="factura-print-area">
        <div style={{background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 16px rgba(0,0,0,.1)",fontFamily:"Arial,sans-serif"}}>
          <div style={{background:"linear-gradient(135deg,#0F2444,#1B3E6E)",padding:"24px 28px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:16}}>
            <div>
              <div style={{color:"#fff",fontWeight:900,fontSize:22,letterSpacing:-0.5}}>🏗️ {empresa.nombre}</div>
              <div style={{color:"rgba(255,255,255,.65)",fontSize:11,marginTop:6,lineHeight:1.8}}>
                {empresa.email}
              </div>
            </div>
            <div style={{textAlign:"right"}}>
          ...
          <div style={{padding:"24px 28px"}}>
            ...
            {factura.notas&&(
              <div style={{background:"#EFF6FF",borderRadius:10,padding:"12px 16px",fontSize:12,color:"#475569",borderLeft:"4px solid #3B82F6"}}>
                <b style={{color:"#1D4ED8"}}>Notas: </b>{factura.notas}
              </div>
            )}
          </div>
        </div>
...
function FacturaForm({ modal, clientes, nextNum, empresa, onClose, onSave, inp }) {
  const f = modal.mode==="edit" ? modal.f : null;
  const snap = f?.cliente_snapshot || {};
  const [form, setForm] = useState({
    numero: f?.numero || nextNum,
    fecha:  f?.fecha  || new Date().toISOString().slice(0,10),
    vencimiento: f?.vencimiento || "Al recibir",
    estado: f?.estado || "pendiente",
    cliente_id: f?.cliente_id || "",
    cliente: snap.nombre ? snap : { nombre:"",nif:"",direccion:"",ciudad:"",cp:"",telefono:"",email:"" },
  ...
  const loadCliente = (cid) => {
    const c = clientes.find(x=>x.id===cid);
    if (c) setForm(p=>({...p,cliente_id:cid,cliente:{nombre:c.nombre,nif:c.nif||"",direccion:c.direccion||"",ciudad:c.ciudad||"",cp:c.cp||"",telefono:c.telefono||"",email:c.email||""}}));
  };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"flex-start",justifyContent:"center",zIndex:50,padding:12,overflowY:"auto"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#F1F5F9",borderRadius:16,width:"100%",maxWidth:560,boxShadow:"0 8px 40px rgba(0,0,0,.3)",marginTop:8,overflow:"hidden",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
        ...
        <div style={{padding:16,maxHeight:"75vh",overflowY:"auto"}}>
          ...
          <Sec title="Cliente">
            {clientes.length>0&&(
              <div style={{marginBottom:10}}>
                <Fld label="Cargar cliente guardado">
                  <select onChange={e=>loadCliente(e.target.value)} style={inp}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <Fld label="Nombre / Razón social"><input style={inp} value={form.cliente.nombre} onChange={e=>setC("nombre",e.target.value)}/></Fld>
          ...
          <Sec title="IVA">
            <Fld label="Tipo de IVA">
              <select value={form.iva} onChange={e=>setForm(p=>({...p,iva:parseInt(e.target.value)}))} style={{...inp,width:140}}>
          ...
          <Sec title="Notas (opcional)">
            <textarea style={{...inp,height:70,resize:"vertical"}} value={form.notas} onChange={e=>setForm(p=>({...p,notas:e.target.value}))} placeholder="Observaciones, condiciones..."/>
          </Sec>
...
function ResumenTab({ totales, trabajadores, obras, facturas, calcFacturaTotals, puestoColor, badge, card, selMonth, selYear, calcWorker, puestos, obrasList, fmt, MONTHS }) {
  const totFact = facturas.reduce((s,f)=>s+calcFacturaTotals(f).total,0);
  const fmt2 = (n) => Number(n).toLocaleString("es-ES",{minimumFractionDigits:2})+"€";
  return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        {[["Peonadas",totales.dias+"","📋","#0F2444"],["Bruto nómina",fmt(totales.bruto),"💶","#0F2444"],["Anticipos",fmt(totales.antic),"💵","#f59e0b"],["A pagar",fmt(totales.pend),"✅","#059669"]].map(([l,v,ic,c])=>(
      ...
      <div style={card}>
        ...
        {[["Pendiente de cobro",facturas.filter(f=>f.estado==="pendiente").reduce((s,f)=>s+calcFacturaTotals(f).total,0),"#F59E0B"],
          ["Cobrado",facturas.filter(f=>f.estado==="cobrada").reduce((s,f)=>s+calcFacturaTotals(f).total,0),"#10B981"],
          ["Total facturado",totFact,"#0F2444"]].map(([l,v,c])=>(
